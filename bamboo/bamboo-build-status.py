...
SERVER = config['url']
PLANS = config['plans']
DEPLOYMENT_PROJECT_ID = config['deployment_project_id']
...
def get_json_build_from_response(branch_key, plan, plan_response, result_response):
    if "planName" in result_response:
        branch_name = '[{}] master'.format(plan_response['projectKey']) if branch_key == plan else result_response[
            'planName']
        return {
...
print('Bamboo')
print('---')
if utils.is_server_up(SERVER):
    get_plans()
    print('---')
    get_deployment_environments()
else:
    print("Sorry, Bamboo seems to be down at the moment")
